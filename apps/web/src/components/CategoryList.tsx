...
function CategoryList({ onSelect, token }: CategoryListProps) {
    ...
    useEffect(() => {
        async function fetchData(status: 'applied' | 'rejected') {
            try {
                ...
                status === 'applied' ? setAppliedJobs(apps.length) : setRejectedJobs(apps.length);
            } catch (error) {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mx-auto gap-5 py-5">
            <button className="card-item flex items-center bg-green-100 border-2 border-green-400 p-6 rounded-lg shadow-sm hover:bg-green-400 transition duration-400 ease-in-out" onClick={() => handleSelect(0)}>
            ...
            <button className="card-item flex items-center bg-yellow-100 border-2 border-yellow-400 p-6 rounded-lg shadow-sm hover:bg-yellow-400 transition duration-400 ease-in-out" onClick={() => handleSelect(1)}>
                <div className="content min-w-24">
            ...
            <button className="card-item flex items-center bg-red-100 border-2 border-red-400 p-6 rounded-lg shadow-sm hover:bg-red-400 transition duration-400 ease-in-out" onClick={() => handleSelect(2)}>
                <div className="content min-w-24">
                    <div className="text-3xl font-semibold text-gray-800 mb-2">{rejectedJobs}</div>
