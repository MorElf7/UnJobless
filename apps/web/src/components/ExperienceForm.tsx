...
import { ExperienceEntry } from '../types/types';
...
const ExperienceForm: React.FC<Props> = ({ nextStep, prevStep, handleArrayChange, signupData }) => {
  ...
  const handleAddEntry = () => {
    if (!entry.position || !entry.company || !entry.location || !entry.description || !entry.startDate || (!entry.current && !entry.endDate)) {
  ...
  return (
    <div className="max-w-md mx-auto space-y-4 relative">
      ...
      <input type="text" placeholder="Company" value={entry.company} onChange={(e) => {
        ...
        <ul className="absolute z-10 w-full bg-white shadow-lg max-h-60 overflow-y-auto border-gray-300 border rounded-md mt-1">
          {companyOptions.map((company, index) => (
            <li key={index} className="p-2 hover:bg-gray-100 cursor-pointer flex justify-between items-center"
                onClick={() => {
                  setEntry({ ...entry, company: company.name, logo: company.logo });
                  setCompanyOptions([]);
                }}
            >
              {company.name}
              <img src={company.logo} alt={`${company.name} logo`} className="w-12 h-7 object-cover rounded-md" />
            </li>
          ))}
        </ul>
      )}
