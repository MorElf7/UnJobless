...
link.href =
  'https://fonts.googleapis.com/css2?family=Nunito:ital,wght@0,200..1000;1,200..1000&family=Ubuntu:ital,wght@0,300;0,400;0,500;0,700;1,300;1,400;1,500;1,700&display=swap';
link.rel = 'stylesheet';
...
(function () {
  ...
  function onUrlChange() {
    ...
    // Add message listener for communication with webpage
    window.addEventListener(
      ...
      event => {
        ...
        if (event.data.type === 'token') {
          chrome.storage.sync.set({ token: event.data.data });
        } else if (event.data.type === 'jid') {
          chrome.storage.sync.set({ jid: event.data.data });
        }
      },
      false,
    );
    ...
    if (/^https?:\/\/boards\.greenhouse\.io\//.test(window.location.href)) {
      ...
      /^https?:\/\/.*\.myworkdayjobs\.com\/.*\/apply/.test(window.location.href) ||
